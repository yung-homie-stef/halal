...
public class FFT_GPU : MonoBehaviour
{
    ...
    void InitializeResources(int size, int anisoLevel)
    {
        ...
        //DisplaceTexture.wrapMode = TextureWrapMode.Repeat;
        //DisplaceTexture.filterMode = FilterMode.Bilinear;
        //DisplaceTexture.useMipMap = false;
    ...
    public void ComputeFFT(LodPrefix lodPrefix, int size, int anisoLevel,
        float domainSize, float windTurbulence, float windSpeed, float windRotation, float timeOffset, List<Material> waterSharedMaterials, Dictionary<ComputeShader, List<int>> waterSharedComputeShaders)
    {
        ...
        foreach (var computeShader in waterSharedComputeShaders)
        {
            if (computeShader.Key == null) continue;
            UpdateComputeParameters(lodPrefix, normalLodSize, domainSize, mipCount, computeShader.Key, computeShader.Value);
        }
    }
