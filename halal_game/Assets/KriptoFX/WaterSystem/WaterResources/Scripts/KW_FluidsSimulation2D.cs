...
public class KW_FluidsSimulation2D : MonoBehaviour
{
    ...
    public RenderTexture prebakedFluidsRT;
    ...
    private const string fluidSimulationShaderName = "Hidden/KriptoFX/Water/FluidSimulation2D";
    ...
    void InitializeTextures(int width, int height)
    {
        for(int i = 0; i < 4; i++)
        {
            ...
            fluidsData[i].DataRT = KW_Extensions.ReinitializeRenderTexture(fluidsData[i].DataRT, width, height, 0, RenderTextureFormat.ARGBFloat, Color.black);
            fluidsData[i].FoamRT = KW_Extensions.ReinitializeRenderTexture(fluidsData[i].FoamRT, width, height, 0, RenderTextureFormat.R8, Color.black);
            fluidsData[i].MRT[0] = fluidsData[i].DataRT.colorBuffer;
            fluidsData[i].MRT[1] = fluidsData[i].FoamRT.colorBuffer;
        }
        lastWidth = width;
        lastHeight = height;
    }
    ...
    void OnDisable()
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                KW_Extensions.ReleaseRenderTextures(fluidsData[i].DataRT, fluidsData[i].FoamRT);
            }
        }
        KW_Extensions.ReleaseRenderTextures(prebakedFluidsRT);
    ...
    {
        if (fluidsMaterial == null) fluidsMaterial = KW_Extensions.CreateMaterial(fluidSimulationShaderName);
    ...
    public void PrebakeSimulation(Vector3 waterPosition, int areaSize, int resolution, float flowSpeed, float foamStrength, string guid)
    {
        ...
        Shader.SetGlobalTexture("KW_Fluids_Lod0", target_lod0.DataRT);
        Shader.SetGlobalTexture("KW_FluidsFoam_Lod0", target_lod0.FoamRT);
    ...
    public void SavePrebakedSimulation(string GUID)
    {
        ...
        var tempRT = new RenderTexture(prebakedFluidsRT.width, prebakedFluidsRT.height, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
        Graphics.Blit(prebakedFluidsRT, tempRT, fluidsMaterial, 1);
    ...
    public void RenderFluids(Camera currentCamera, Vector3 waterPosition, int areaSize, int resolution, float flowSpeed, float foamStrength, string GUID)
    {
        ...
        var target_lod0 = RenderFluidLod(fluidsData[2], fluidsData[3], flowSpeed * 1.0f, areaSize, foamStrength * 5f, (Vector3)offset, areaPosition, true, target_lod1.DataRT);
      
        Shader.SetGlobalTexture("KW_Fluids_Lod0", target_lod0.DataRT);
        Shader.SetGlobalTexture("KW_FluidsFoam_Lod0", target_lod0.FoamRT);

        Shader.SetGlobalTexture("KW_Fluids_Lod1", target_lod1.DataRT);
        Shader.SetGlobalTexture("KW_FluidsFoam_Lod1", target_lod1.FoamRT);
        ...
        bool canUseNextLod, RenderTexture nextLod = null)
    {
        ...
        Graphics.Blit(source.DataRT, fluidsMaterial, 0);
        return target;
    }
}
