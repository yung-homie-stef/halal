Shader "Hidden/KriptoFX/Water/DynamicWaves"
{
	Properties
	{
...
#include "KW_WaterVariables.cginc"
	...
	v2f vert(appdata_img v)
	{
		...
		o.uv[0] = float4(v.texcoord.xy + KW_LastAreaOffset.xz, 0, 0);
		o.uv[1] = float4(v.texcoord.xy + size.xz + KW_AreaOffset.xz, 0, 0); //0.004, 0      right
		o.uv[2] = float4(v.texcoord.xy + size.yz + KW_AreaOffset.xz, 0, 0); //-0.004, 0     left
		o.uv[3] = float4(v.texcoord.xy + size.zx + KW_AreaOffset.xz, 0, 0); //0, 0.004      up
		o.uv[4] = float4(v.texcoord.xy + size.zy + KW_AreaOffset.xz, 0, 0); //0, -0.004     
		o.uv[5] = float4(v.texcoord.xy, 0, 0);

		float2 worldUV = v.texcoord.xy * KW_DynamicWavesAreaSize - KW_DynamicWavesAreaSize * 0.5;
	...
	{
		FragmentOutput o;
		if (i.uv[1].x < 0.01 || i.uv[1].x > 0.99 || i.uv[1].y < 0.01 || i.uv[1].y > 0.99)
		{
			o.dest0 = 0.0;
			o.dest1 = 0.5;
			return o;
